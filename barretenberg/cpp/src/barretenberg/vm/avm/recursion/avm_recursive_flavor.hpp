      ...
      public:
        ...
        VerificationKey(CircuitBuilder* builder, const std::shared_ptr<NativeVerificationKey>& native_key)
        {
            ...
            for (auto [native_comm, comm] : zip_view(native_key->get_all(), this->get_all())) {
                info("JEAMON - NATIVE COMMIT:", native_comm);
                info("JEAMON - INITIAL COMMIT:", comm);

                // Figure out whether the EC relation y^2 = x^3 + 3 (mod. q)
                fq rhs = (native_comm.x * native_comm.x) * native_comm.x;
                rhs = rhs + 3;
                info("RHS: ", rhs);
                fq lhs = native_comm.y * native_comm.y;
                info("LHS: ", lhs);

                comm = Commitment::from_witness(builder, native_comm);
                info("JEAMON - COMMIT:", comm);
            }
            info("AFTER JEAMON - TESTING");
        }
    };
